...
void GastonTarry::doStep() {
    ...
    if(maze->isCrossing(coords)){
        ...
        if((maze->getPosition(left) == ' ' || maze->getPosition(left) == '.') && map[left.posx][left.posy] != STOP && map[left.posx][left.posy] != BACK){
            turn(LEFT);
            coords = left;
            map[coords.posx][coords.posy] = STOP;
        }else if((maze->getPosition(right) == ' ' || maze->getPosition(right) == '.') && map[right.posx][right.posy] != STOP && map[right.posx][right.posy] != BACK){
            turn(RIGHT);
            coords = right;
            map[coords.posx][coords.posy] = STOP;
        }else if((maze->getPosition(forward) == ' ' || maze->getPosition(forward) == '.') && map[forward.posx][forward.posy] != STOP && map[forward.posx][forward.posy] != BACK){
            coords = forward;
            map[coords.posx][coords.posy] = STOP;
        }else if((maze->getPosition(left) == ' ' || maze->getPosition(left) == '.') && map[left.posx][left.posy] == BACK){
            turn(LEFT);
            coords = left;
            map[coords.posx][coords.posy] = STOP;
        }else if((maze->getPosition(right) == ' ' || maze->getPosition(right) == '.') && map[right.posx][right.posy] == BACK){
            turn(RIGHT);
            coords = right;
            map[coords.posx][coords.posy] = STOP;
        }else if((maze->getPosition(forward) == ' ' || maze->getPosition(forward) == '.') && map[forward.posx][forward.posy] == BACK){
            coords = forward;
            map[coords.posx][coords.posy] = STOP;
        }
    }else{
